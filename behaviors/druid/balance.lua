...
local function BalanceCombat()
  ...
  if target:TimeToDeath() > 5 then
    if Me:IsFacing(target) and not target:HasVisibleAura("Insect Swarm") and Spell.InsectSwarm:CastEx(target) then return end
    if Me:IsFacing(target) and not target:HasVisibleAura("Moonfire") and Spell.Moonfire:CastEx(target) then return end
  end

  if Me:IsMoving() then return end
  if Me:IsFacing(target) and Spell.Starfire:CastEx(target) then return end
end
